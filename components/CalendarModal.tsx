        ...
        >
          ...
          <div className="px-8 py-6 flex items-center justify-between bg-teal-500/5">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-black tracking-tight">旅程记事</h2>
            </div>
          ...
          <div className="p-8 overflow-y-auto flex-grow no-scrollbar">
            {!isAuthenticated ? (
              <div className="text-center py-12 text-slate-400">请先登录以保存您的旅程日程</div>
            ) : (
              <div className="space-y-6">
                <div className="flex justify-between items-center">
                  <span className="text-xs font-black text-slate-400 uppercase tracking-widest">我的安排</span>
                  <button 
                ...
                <AnimatePresence>
                  {showAddForm && (
                    <motion.form 
                      ...
                      onSubmit={handleAdd}
                      className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-2xl space-y-4 overflow-hidden border border-slate-100 dark:border-slate-700"
                    >
                      <input 
                        required
                        className="w-full bg-white dark:bg-slate-800 border-none rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-teal-500"
                        placeholder="目的地 / 标题"
                ...
                <div className="space-y-3">
                  {schedules.map(s => (
                    <div key={s.id} className="group p-5 bg-white dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800 hover:border-teal-500/30 transition-all flex justify-between items-center">
                      <div className="flex items-center gap-4">
                        <div className="text-center bg-teal-50 dark:bg-teal-900/30 px-3 py-1.5 rounded-xl border border-teal-100 dark:border-teal-800">
                          <div className="text-[10px] font-black text-teal-600 uppercase leading-none mb-1">{s.schedule_date.split('-')[1]}月</div>
                          <div className="text-lg font-black leading-none">{s.schedule_date.split('-')[2]}</div>
                        </div>
                        <div>
                          <h4 className="font-bold text-sm">{s.title}</h4>
                          <p className="text-[10px] text-slate-400 mt-0.5">{s.description || '暂无详细描述'}</p>
                        </div>
                      </div>
                  ...
                  {schedules.length === 0 && !isLoading && (
                    <div className="py-12 text-center opacity-30 italic text-sm">暂无旅程安排</div>
                  )}
                </div>
              </div>
            )}
