          ...
          >
            <div className="flex justify-between items-center p-6 border-b border-teal-500/5">
              <div className="flex items-center gap-2">
                {view === 'list' && (
                  <button onClick={() => setView('form')} className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl text-slate-400 dark:text-slate-300">
              ...
              <div className="flex items-center gap-1">
                {view === 'form' && (
                  <button onClick={() => { setCurrentPage(1); setView('list'); }} className="p-2 text-teal-600 hover:bg-teal-50 dark:hover:bg-teal-900/40 rounded-xl transition-colors">
                ...
                {view === 'form' ? (
                      ...
                      <>
                        {!isAuthenticated ? (
                          <div className="flex flex-col items-center justify-center py-10 px-4 text-center">
                            <div className="w-16 h-16 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center mb-4 text-slate-400 dark:text-slate-300">
                              <Lock className="w-8 h-8" />
                            </div>
                            <h4 className="font-black text-slate-800 dark:text-white mb-2 tracking-tight">仅限登录用户</h4>
                            <p className="text-xs text-slate-500 dark:text-slate-300 mb-6 leading-relaxed">
                          ...
                          <form onSubmit={handleSubmit} className="space-y-4">
                            <textarea
                              ...
                              placeholder="写下您的想法..."
                              className="w-full h-32 p-4 rounded-2xl bg-slate-100 dark:bg-slate-800 border-none focus:ring-2 focus:ring-teal-500 text-sm resize-none dark:text-white placeholder-slate-400 dark:placeholder-slate-500"
                              maxLength={500}
                            />
                  ...
                  <motion.div key="list" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="space-y-4">
                    {isLoadingList ? (
                      ...
                      <>
                          ...
                          {feedbacks.map((item, idx) => (
                            <motion.div key={idx} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }} className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800">
                              <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className="text-[10px] font-black text-slate-500 dark:text-teal-400 uppercase tracking-widest">
                                  {item.username || '匿名游客'}
                                </span>
                              </div>
                        ...
                        {totalPages > 1 && (
                          <div className="flex items-center justify-between pt-4 pb-2">
                            <button disabled={currentPage === 1} onClick={() => handlePageChange(currentPage - 1)} className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-300 disabled:opacity-30">
                              <ChevronLeft className="w-4 h-4" />
                            </button>
                            <span className="text-[10px] font-black text-slate-400 dark:text-slate-300 uppercase tracking-widest">PAGE {currentPage} / {totalPages}</span>
                            <button disabled={currentPage === totalPages} onClick={() => handlePageChange(currentPage + 1)} className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-300 disabled:opacity-30">
                      ...
                      <div className="text-center py-20 opacity-30 italic text-sm text-slate-500 dark:text-slate-400">暂无反馈记录</div>
                    )}
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
            <div className="px-6 py-4 bg-slate-50/50 dark:bg-slate-800/20 text-[10px] text-center font-bold text-slate-400 dark:text-slate-300 uppercase tracking-widest border-t border-teal-500/5">
              人人参与 · 共建华夏游
            </div>
          </motion.div>
        )}
