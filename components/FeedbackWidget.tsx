      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl rounded-[2.5rem] shadow-2xl border border-teal-500/20 w-80 sm:w-96 mb-4 pointer-events-auto flex flex-col overflow-hidden max-h-[600px]"
          >
            <div className="flex justify-between items-center p-6 border-b border-teal-500/5">
              <div className="flex items-center gap-2">
                {view === 'list' && (
                  <button onClick={() => setView('form')} className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl text-slate-400">
              ...
              <div className="flex items-center gap-1">
                {view === 'form' && (
                  <button onClick={() => { setCurrentPage(1); setView('list'); }} className="p-2 text-teal-600 hover:bg-teal-50 dark:hover:bg-teal-900/20 rounded-xl">
                    <List className="w-5 h-5" />
                  </button>
                )}
                <button onClick={() => setIsOpen(false)} className="p-2 text-slate-400 hover:text-red-500 rounded-xl">
                  <X className="w-5 h-5" />
                </button>
              </div>
            </div>
                        ...
                        {!isAuthenticated ? (
                          ...
                          <form onSubmit={handleSubmit} className="space-y-4">
                            <textarea
                              ...
                              placeholder="写下您的想法..."
                              className="w-full h-32 p-4 rounded-2xl bg-slate-100 dark:bg-slate-800 border-none focus:ring-2 focus:ring-teal-500 text-sm resize-none dark:text-white"
                              maxLength={500}
                            />
                            <button type="submit" disabled={isSubmitting || !content.trim()} className="w-full py-4 bg-teal-500 hover:bg-teal-600 disabled:opacity-50 text-white rounded-2xl font-black text-sm shadow-lg flex items-center justify-center gap-2">
                      ...
                      <>
                        <div className="space-y-4">
                          {feedbacks.map((item, idx) => (
                            <motion.div key={idx} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }} className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-800">
                        ...
                        {totalPages > 1 && (
                          <div className="flex items-center justify-between pt-4 pb-2">
                            <button disabled={currentPage === 1} onClick={() => handlePageChange(currentPage - 1)} className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-500 disabled:opacity-30">
                              <ChevronLeft className="w-4 h-4" />
                            </button>
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">PAGE {currentPage} / {totalPages}</span>
                            <button disabled={currentPage === totalPages} onClick={() => handlePageChange(currentPage + 1)} className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-500 disabled:opacity-30">
