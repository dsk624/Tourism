...
export const WeatherWidget: React.FC = () => {
  ...
  return (
    <div className="fixed top-20 right-4 md:top-24 z-30">
      <motion.div 
        ref={widgetRef}
        layout
        onClick={() => !isExpanded && setIsExpanded(true)}
        transition={{ type: "spring", stiffness: 300, damping: 25 }}
        className={`bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border border-white/20 dark:border-slate-700 rounded-3xl shadow-2xl cursor-pointer overflow-hidden relative flex flex-col ${isExpanded ? 'w-80 sm:w-96' : 'w-auto'}`}
      >
        <motion.div 
          layout
          className={`transition-all duration-300 ${isExpanded ? 'p-5' : 'px-4 py-3 hover:bg-white dark:hover:bg-slate-800'}`}
        >
          {/* Compact View */}
          {!isExpanded && (
             <motion.div 
          ...
          <AnimatePresence mode="popLayout">
            {isExpanded && (
              ...
              >
                ...
                {weather && (
                  <motion.div 
                    className="grid grid-cols-2 sm:grid-cols-3 gap-2.5 text-xs font-medium text-slate-600 dark:text-slate-300"
                ...
                <div 
                   ...
                   className="flex justify-center pt-2 cursor-pointer opacity-50 hover:opacity-100 transition-opacity"
                >
